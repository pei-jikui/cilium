...
func (h *handler) NodeAdd(n node.Node) error {
	select {
...
func (h *handler) NodeUpdate(o, n node.Node) error {
	select {
	case h.C <- &peerpb.ChangeNotification{
		Name:    n.Fullname(),
		Address: nodeAddress(n),
...
func (h *handler) NodeDelete(n node.Node) error {
	select {
	case h.C <- &peerpb.ChangeNotification{
		Name:    n.Fullname(),
		Address: nodeAddress(n),
...
func (h handler) NodeValidateImplementation(_ node.Node) error {
	// no-op
	return nil
}
...
func nodeAddress(n node.Node) string {
